...
The Voyage client is imported lazily so unit tests can mock this module without
...
async def embed_batch(texts: list[str], model: str) -> list[list[float]]:
    ...
    The implementation calls the sync Voyage SDK in a worker thread so callers
    can stay async.
    """
    if not texts:
        return []
